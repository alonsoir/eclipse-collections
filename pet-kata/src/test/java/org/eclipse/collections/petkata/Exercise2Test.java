...
public class Exercise2Test extends PetDomainForKata
{
    ...
    public void howManyPeopleHaveCats()
    {
        ...
        int count = this.people.count(predicate);
    ...
    public void findMarySmith()
    {
        ...
        Predicate<Person> predicate = person -> person.getFirstName().equals("Mary") && person.getLastName().equals("Smith");
    ...
    {
    	Predicate <Person> predicate = person->person.hasPet(PetType.BIRD) || 
    									       person.hasPet(PetType.CAT) || 
    									       person.hasPet(PetType.DOG) || 
    									       person.hasPet(PetType.HAMSTER) || 
    									       person.hasPet(PetType.SNAKE) ||
    									       person.hasPet(PetType.TURTLE);
    	MutableList<Person> petPeople = this.people.select(predicate); // replace with only the pet owners
    ...
    {
        Function<Person,String> function = person -> person.getFirstName();
        MutableList<String> firstNames = this.people.collect(function);
    ...
    public void getPeopleWithCatsRefactor()
    {
		//use method reference, NOT lambdas, to solve the problem below
    	Predicate<Person> predicate = person -> person.hasPet(PetType.CAT);
    	MutableList<Person> peopleWithCatsMethodRef = this.people.select(predicate);
    ...
    public void getPeopleWithoutCatsRefactor()
    {
        //use method reference, NOT lambdas, to solve the problem below
    	Predicate <Person> predicate = person -> person.hasPet(PetType.CAT);
        MutableList<Person> peopleWithoutCatsMethodRef = this.people.reject(predicate);
